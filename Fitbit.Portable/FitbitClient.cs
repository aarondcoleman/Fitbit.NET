...
namespace Fitbit.Api.Portable
{
    public class FitbitClient : IFitbitClient
    {
        ...
        public async Task<List<ActiveZoneMinutesIntraday>> GetActiveZoneMinutesIntradayAsync(DateTime startDate, DateTime? endDate = null, DataResolution resolution = DataResolution.OneMinute)
        {
            ...
            if (endDate == null)
            {
                apiCall = FitbitClientHelperExtensions.ToFullUrl("/1/user/{0}/activites/activeZoneMinutes/date/{1}/{2}.json", args: new object[] { startDate.ToFitbitFormat(), resolution.GetStringValue() });
            }
            else
            {
                apiCall = FitbitClientHelperExtensions.ToFullUrl("/1/user/{0}/activites/activeZoneMinutes/date/{1}/{2}/{3}.json", args: new object[] { startDate.ToFitbitFormat(), endDate.Value.ToFitbitFormat(), resolution.GetStringValue() });
            }
