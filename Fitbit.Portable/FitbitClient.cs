...
namespace Fitbit.Api.Portable
{
    public class FitbitClient : IFitbitClient
    {
        ...
        private async Task<HeartActivitiesIntradayTimeSeries> ProcessHeartRateIntradayTimeSeries(string url)
        {
            ...
            var fitbitResponse = seralizer.GetHeartIntradayTimeSeries(responseBody);
        ...
        /// <param name="encodedUserId">Optional: Encoded id of the user.</param>
        /// <returns></returns>
        public async Task<HeartActivitiesIntradayTimeSeries> GetHeartRateIntraday(DateTime date, HeartRateResolution resolution, string encodedUserId = default(string))
        {
            return await GetHeartRateIntradayTimeSeries(date, date, resolution, TimeSpan.Parse("00:00:00"), TimeSpan.Parse("23:59:59"), encodedUserId);
        }
        ...
        /// </summary>
        /// <param name="date">The start date and time of series.</param>
        /// /// <param name="startTime">The start of the period, in the format HH:mm:ss.</param>
        /// <param name="endTime">The end of the period, in the format HH:mm:ss</param>
        /// <param name="resolution">Number of data points to include.</param>
        /// <param name="encodedUserId">Optional: Encoded id of the user.</param>
        /// <returns></returns>
        public async Task<HeartActivitiesIntradayTimeSeries> GetHeartRateIntraday(DateTime date, TimeSpan startTime, TimeSpan endTime, HeartRateResolution resolution, string encodedUserId = default(string))
        {
            return await GetHeartRateIntradayTimeSeries(date, date, resolution, startTime, endTime, encodedUserId);
        }

        /// <summary>
        /// Requests the Intraday Heart Rate Time Series for a specific date.
        /// </summary>
        /// <param name="startDate">The start date and time of series.</param>
        /// <param name="endDate">The end date and time of series. Max 24 hour time period.</param>
        /// <param name="resolution">Number of data points to include.</param>
        /// <param name="encodedUserId">Optional: Encoded id of the user.</param>
        /// <returns></returns>
        public async Task<HeartActivitiesIntradayTimeSeries> GetHeartRateIntraday(DateTime startDate, DateTime endDate, HeartRateResolution resolution, string encodedUserId = default(string))
        {
            if (startDate.AddDays(1) <= endDate)
            {
                throw new FitbitException("startDate and endDate must be within a 24 hour period", new List<ApiError>());
            }
            ...
            {
                startTime = TimeSpan.Parse(startDate.ToString("HH:mm:ss")); //ToString is used to make sure we are in military time
                endTime = TimeSpan.Parse(endDate.ToString("HH:mm:ss"));
            }


            return await GetHeartRateIntradayTimeSeries(startDate, endDate, resolution, startTime, endTime, encodedUserId);
        }
        ...
        /// </summary>
        /// <param name="startDate">The date, in the format yyyy-MM-dd or today.</param>
        /// <param name="endDate">The end date of the range.</param>
        /// <param name="resolution">Number of data points to include.</param>
        /// <param name="startTime">The start of the period, in the format HH:mm:ss.</param>
        /// <param name="endTime">The end of the period, in the format HH:mm:ss. Max 24 hour time period.</param>
        /// <param name="encodedUserId">encoded user id, can be null for current logged in user</param>
        /// <returns>List of HeartActivitiesIntraday</returns>
        public async Task<HeartActivitiesIntradayTimeSeries> GetHeartRateIntradayTimeSeries(DateTime startDate, DateTime endDate, HeartRateResolution resolution, TimeSpan startTime, TimeSpan endTime, string encodedUserId = default(string))
        {
            if (startDate != endDate && startTime.Add(endTime).Days > 0)
            {
                throw new FitbitException("startDate/startTime and endDate/endTime must be within a 24 hour period", new List<ApiError>());
            }

            string resolutionText = GetHeartRateResolution(resolution);

            string apiPath = $"1.1/user/{encodedUserId}/activities/heart/date/{startDate:yyyy-MM-dd}/{endDate:yyyy-MM-dd}/{resolutionText}/time/{startTime}/{endTime}.json";
            string apiCall = FitbitClientHelperExtensions.ToFullUrl(apiPath);

            return await ProcessHeartRateIntradayTimeSeries(apiCall);
        }
